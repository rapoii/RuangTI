# Modul Komprehensif: Shop Floor Control (SFC), Dispatching Rules (SPT, EDD, CR), & WIP Tracking
**Nomor Modul:** [357]  
**Domain Keahlian:** Rekayasa Sistem & Teknik Industri Terpadu (Industrial & Systems Engineering)  
**Sumber Referensi:** *Production and Operations Analysis (Steven Nahmias, Tava Lennon Olsen), EJOR (2024)*.

---

## 1. Landasan Teori & Tinjauan Konseptual
Modul ini menyajikan pendekatan fundamental dan metodologi tingkat lanjut dalam domain **Shop Floor Control (SFC), Dispatching Rules (SPT, EDD, CR), & WIP Tracking**. Di era transformasi industri kontemporer (Industry 4.0 & Society 5.0), integrasi antara pemodelan matematis, otomasi komputasi, dan optimasi proses menjadi pilar utama peningkatan produktivitas, efisiensi sumber daya, dan ketahanan operasional (*operational resilience*).

### Pokok Bahasan & Prinsip Utama:
- **Cakupan Inti**: Shop Floor Dispatching Rules, Critical Ratio (CR), Minimum Slack per Operation, Input/Output Control (I/OC), RFID Barcode Work-in-Process Tracking.
...
```python
# Algoritma Solusi Terapan untuk Shop Floor Control (SFC), Dispatching Rules (SPT, EDD, CR), & WIP Tracking
import numpy as np
...
**Konteks Penerapan**: Studi Kasus: Pengendalian Aliran WIP Lini Fabrikasi Logam dengan Dynamic Critical Ratio Dispatching.
...
1. Production and Operations Analysis (Steven Nahmias, Tava Lennon Olsen), EJOR (2024).
