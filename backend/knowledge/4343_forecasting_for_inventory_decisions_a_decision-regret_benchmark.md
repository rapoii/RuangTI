# 4343 — Supply Chain Management & Logistics Engineering: Forecasting for Inventory Decisions: A Decision-regret Benchmark for Perishability-aware Multi-Echelon Retail Replenishment using the M5/Walmart Data

**Domain:** Supply Chain Management & Logistics Engineering  
**Topik Spesifik:** Forecasting for Inventory Decisions: A Decision-regret Benchmark for Perishability-aware Multi-Echelon Retail Replenishment using the M5/Walmart Data  
**Jurnal & Sitasi Utama:** Pan Li (2024). *Peer-Reviewed Journal*. DOI: [10.2139/ssrn.7051299](10.2139/ssrn.7051299)  
**Sitasi Pendukung:** Liudong Gu, Yifan Zhou, Zhisheng Zhang (2020). *2020 Global Reliability and Prognostics and Health Management (PHM-Shanghai)*. DOI: [10.1109/phm-shanghai49105.2020.9281010](10.1109/phm-shanghai49105.2020.9281010)  

---

## 1. Pendahuluan dan Konteks Industri

Dalam lanskap operasional modern, disiplin Supply Chain Management & Logistics Engineering memegang peranan krusial dalam merancang, mengintegrasikan, dan memperbaiki efisiensi sistem produksi terintegrasi. Dinamika persaingan global menuntut integrasi antara alokasi sumber daya manusia, material, informasi, dan permesinan guna mencapai biaya operasional yang minimal serta keandalan output yang optimal. Artikel penelitian rujukan dari Pan Li (2024) dalam jurnal *Peer-Reviewed Journal* (DOI: 10.2139/ssrn.7051299) menyajikan pendekatan sistematis terhadap pemecahan masalah operasional berskala industri.
...
> "Forecasting models used in retail replenishment are commonly selected by point-forecast accuracy, even though downstream decisions depend on stockout exposure, spoilage, lead time, and upstream aggregation. This paper develops a decision-regret benchmark for evaluating demand forecasts as inputs to perishability-aware multi-echelon inventory decisions. Using the public M5/Walmart daily sales benchmark, the study evaluates statistical, intermittentdemand, and machine-learning forecasting specifications under rolling-origin 28-day backtesting. Unlike accuracyonly evaluations, the proposed framework converts each forecast into a forecast-plus-safety-stock replenishment action and then measures store-level fill rate, stockout rate, spoilage rate, resilience cost, and distribution-center aggregation cost. The paper introduces a Decision Regret Index (DRI), defined as the percentage excess inventory resilience cost of a method relative to the best method under the same operating scenario. It also reports an Inventory-Adjusted Forecast Score (IAFS) and tests alternative decision weights. Empirically, the full 30,490 bottom-level itemstore panel shows that intermittent-demand TSB and a 28-day moving average remain difficult-to-beat operational benchmarks, with TSB achieving the lowest average resilience cost. On the stratified 3,000-series machine-learning panel, LightGBM variants reduce decision regret under baseline operating costs, but the ranking changes under accuracy-heavy, ser"

Penelitian pendukung oleh Liudong Gu, Yifan Zhou, Zhisheng Zhang (2020) yang dipublikasikan dalam *2020 Global Reliability and Prognostics and Health Management (PHM-Shanghai)* menggarisbawahi bahwa integrasi model analitis kuantitatif ke dalam lingkungan manufaktur dan logistik secara empiris memitigasi risiko kegagalan proses. Implementasi sistem ini mengharuskan arsitektur data industri terstruktur yang menghubungkan sistem *Enterprise Resource Planning* (ERP), *Manufacturing Execution Systems* (MES), dan sensor lantai pabrik secara deterministik dan terukur.
