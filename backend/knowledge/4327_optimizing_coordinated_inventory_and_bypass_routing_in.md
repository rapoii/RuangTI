# 4327 — Supply Chain Management & Logistics Engineering: Optimizing Coordinated Inventory and Bypass Routing in Multi-Product Three-Echelon Supply Chains via Multi-Agent Deep Reinforcement Learning

**Domain:** Supply Chain Management & Logistics Engineering  
**Topik Spesifik:** Optimizing Coordinated Inventory and Bypass Routing in Multi-Product Three-Echelon Supply Chains via Multi-Agent Deep Reinforcement Learning  
**Jurnal & Sitasi Utama:** Vinh Thanh Ho (2024). *Peer-Reviewed Journal*. DOI: [10.2139/ssrn.6921598](10.2139/ssrn.6921598)  
**Sitasi Pendukung:** Yifan Zhou, Kai Guo, Cheng Yu (2024). *Applied Mathematical Modelling*. DOI: [10.1016/j.apm.2023.10.039](10.1016/j.apm.2023.10.039)  

---

## 1. Pendahuluan dan Konteks Industri

Dalam lanskap operasional modern, disiplin Supply Chain Management & Logistics Engineering memegang peranan krusial dalam merancang, mengintegrasikan, dan memperbaiki efisiensi sistem produksi terintegrasi. Dinamika persaingan global menuntut integrasi antara alokasi sumber daya manusia, material, informasi, dan permesinan guna mencapai biaya operasional yang minimal serta keandalan output yang optimal. Artikel penelitian rujukan dari Vinh Thanh Ho (2024) dalam jurnal *Peer-Reviewed Journal* (DOI: 10.2139/ssrn.6921598) menyajikan pendekatan sistematis terhadap pemecahan masalah operasional berskala industri.
...
> "We study capacitated, multi-product, three-echelon supply chain inventory control with stochastic seasonal demand, uncertain lead times, and replenishment through either standard hierarchical paths or factory-to-warehouse bypass routes. Real supply chains are organizationally decentralized; however, existing multi-agent deep reinforcement learning (MADRL) frameworks have not yet incorporated bypass routing as an explicit decision variable. We formulate the problem as a cooperative Markov game and decompose decisions among fixed decision-type agents for production, factory-to-distribution-center shipment, distribution, and routing. The routing agent selects binary bypass decisions for each warehouse, while quantity agents choose continuous production and shipment volumes. Methodologically, we compare independent PPO (IPPO), in which agents learn local critics, with multi-agent PPO (MAPPO), which uses a shared centralized critic. Moreover, we combine both these algorithms with two pipeline-observation designs: full arc-level information (Full) and compact imminent-arrival aggregation (Redu). Experiments on benchmark networks with up to 25 nodes, divergent and general topologies, and deterministic or stochastic lead times show that MADRL clearly outperforms static-policy and expected-value baselines as well as single-agent approaches. IPPO-Redu achieves the strongest overall performance, whereas MAPPO&amp;apos;s centralized critic provides no consistent advantage. The reduced pi"

Penelitian pendukung oleh Yifan Zhou, Kai Guo, Cheng Yu (2024) yang dipublikasikan dalam *Applied Mathematical Modelling* menggarisbawahi bahwa integrasi model analitis kuantitatif ke dalam lingkungan manufaktur dan logistik secara empiris memitigasi risiko kegagalan proses. Implementasi sistem ini mengharuskan arsitektur data industri terstruktur yang menghubungkan sistem *Enterprise Resource Planning* (ERP), *Manufacturing Execution Systems* (MES), dan sensor lantai pabrik secara deterministik dan terukur.
