# 5068 — Simulation Modeling & Digital Twins: An Agent-Based Simulation and Optimization Framework for Resource Allocation Decisions in Bio-Based Isopropanol Supply Chain

**Domain:** Simulation Modeling & Digital Twins  
**Topik Spesifik:** An Agent-Based Simulation and Optimization Framework for Resource Allocation Decisions in Bio-Based Isopropanol Supply Chain  
**Jurnal & Sitasi Utama:** Ching-Mei Wen, Marianthi Ierapetritou (2024). *Peer-Reviewed Journal*. DOI: [10.2139/ssrn.6379640](10.2139/ssrn.6379640)  
**Sitasi Pendukung:** Rob Bemthuis, Martijn Mes, Maria-Eugenia Iacob (2020). *2020 Winter Simulation Conference (WSC)*. DOI: [10.1109/wsc48552.2020.9383956](10.1109/wsc48552.2020.9383956)  

---

## 1. Pendahuluan dan Konteks Industri

Dalam lanskap operasional modern, disiplin Simulation Modeling & Digital Twins memegang peranan krusial dalam merancang, mengintegrasikan, dan memperbaiki efisiensi sistem produksi terintegrasi. Dinamika persaingan global menuntut integrasi antara alokasi sumber daya manusia, material, informasi, dan permesinan guna mencapai biaya operasional yang minimal serta keandalan output yang optimal. Artikel penelitian rujukan dari Ching-Mei Wen, Marianthi Ierapetritou (2024) dalam jurnal *Peer-Reviewed Journal* (DOI: 10.2139/ssrn.6379640) menyajikan pendekatan sistematis terhadap pemecahan masalah operasional berskala industri.
...
> "This work studies resource allocation decisions in a sugar-based bio-isopropanol (IPA) supply chain operating under competing food and chemical markets. Unlike conventional models with fixed or exogenous prices, market prices here arise from strategic interactions among producers and consumers. Sugar can be sold directly or converted to bio-IPA, creating competition for a shared intermediate resource whose value changes over time. A centralized double-sided auction with Q-learning agents is used to model adaptive pricing and bidding behavior. As the objective function is difficult to be expressed analytically and depends on market dynamics over time, a simulation-based optimization framework is adopted. Particle swarm optimization (PSO) is used to search for time-dependent sugar allocation strategies, with the market simulation acting as a black-box evaluator. A topology-informed PSO variant is also examined to improve search performance.,A multi-period case study shows that allowing prices to adjust through learning results in different allocation strategies and profit outcomes compared with fixed-price assumptions. In particular, learning-based price formation shifts the optimal policy toward bio-IPA production in high-IPA-demand cases while maintaining a nonzero share of direct sugar sales to preserve flexibility under time-varying prices and penalties. The topology-informed PSO identifies higher-profit allocation strategies than standard PSO across all demand scenarios, w"

Penelitian pendukung oleh Rob Bemthuis, Martijn Mes, Maria-Eugenia Iacob (2020) yang dipublikasikan dalam *2020 Winter Simulation Conference (WSC)* menggarisbawahi bahwa integrasi model analitis kuantitatif ke dalam lingkungan manufaktur dan logistik secara empiris memitigasi risiko kegagalan proses. Implementasi sistem ini mengharuskan arsitektur data industri terstruktur yang menghubungkan sistem *Enterprise Resource Planning* (ERP), *Manufacturing Execution Systems* (MES), dan sensor lantai pabrik secara deterministik dan terukur.
...
```python
# Algoritma Solusi Terapan untuk Simulation Modeling & Digital Twins
import numpy as np
...
def execute_industrial_solver(parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    sistem Simulation Modeling & Digital Twins dan analisis efisiensi proses industri.
    """
