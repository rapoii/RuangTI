# 4727 — Facilities Planning & Material Handling: Bridging Event-Based and StorAge Selection Approaches to Backward Travel Time Distributions

**Domain:** Facilities Planning & Material Handling  
**Topik Spesifik:** Bridging Event-Based and StorAge Selection Approaches to Backward Travel Time Distributions  
**Jurnal & Sitasi Utama:** Simon Hoeg (2024). *Peer-Reviewed Journal*. DOI: [10.22541/essoar.15004335/v2](10.22541/essoar.15004335/v2)  
**Sitasi Pendukung:** Dhanashri M. Biradar, Raushan Kumar, Govind Singh Patel (2026). *International Journal of Computer Information Systems and Industrial Management Applications*. DOI: [10.70917/ijcisim-2026-4494](10.70917/ijcisim-2026-4494)  

---

## 1. Pendahuluan dan Konteks Industri

Dalam lanskap operasional modern, disiplin Facilities Planning & Material Handling memegang peranan krusial dalam merancang, mengintegrasikan, dan memperbaiki efisiensi sistem produksi terintegrasi. Dinamika persaingan global menuntut integrasi antara alokasi sumber daya manusia, material, informasi, dan permesinan guna mencapai biaya operasional yang minimal serta keandalan output yang optimal. Artikel penelitian rujukan dari Simon Hoeg (2024) dalam jurnal *Peer-Reviewed Journal* (DOI: 10.22541/essoar.15004335/v2) menyajikan pendekatan sistematis terhadap pemecahan masalah operasional berskala industri.
...
> "Event-based backward travel time distributions (bTTDs) derived from tracer impulse responses provide event-scale insight into catchment transport, while StorAge Selection (SAS) functions describe integrated transport dynamics at the system scale. Despite their shared conceptual basis, their relationship across temporal scales remains poorly understood. Here, we develop a unified framework to directly compare event-based and SASderived bTTDs. Event-based bTTDs are constructed from individual tracer impulse response functions and expressed in event-order space, while SAS-based bTTDs are projected onto the same representation using storage-age information. Building on this, we introduce a hybrid formulation that adaptively combines both sources of information. The framework is evaluated using controlled numerical experiments and a multiyear field tracer data set. Event-based bTTDs capture short-term variability but are sensitive to noise and incomplete responses, whereas SAS-based bTTDs provide smoother but temporally aggregated representations. The hybrid approach consistently improves agreement with reference bTTDs, reducing distributional discrepancies and stabilizing mean travel time estimates. Additional analyses show that increasing the complexity of SAS representations, such as through time-varying formulations, does not necessarily improve performance. Instead, stationary SAS functions provide a more robust basis for hybridization, indicating that the SAS component prima"

Penelitian pendukung oleh Dhanashri M. Biradar, Raushan Kumar, Govind Singh Patel (2026) yang dipublikasikan dalam *International Journal of Computer Information Systems and Industrial Management Applications* menggarisbawahi bahwa integrasi model analitis kuantitatif ke dalam lingkungan manufaktur dan logistik secara empiris memitigasi risiko kegagalan proses. Implementasi sistem ini mengharuskan arsitektur data industri terstruktur yang menghubungkan sistem *Enterprise Resource Planning* (ERP), *Manufacturing Execution Systems* (MES), dan sensor lantai pabrik secara deterministik dan terukur.
