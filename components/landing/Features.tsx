...
import { Factory, Boxes, RotateCcw, Timer, Cpu, Binary, Layers, LineChart } from "lucide-react";
...
export function Features() {
  return (
    <section id="features" className="py-20 px-4 sm:px-6 max-w-6xl mx-auto border-t border-border/40">
        ...
        {PILLARS.map((p, idx) => {
          ...
          return (
            <motion.div
              ...
              className="p-6 sm:p-7 rounded-2xl bg-surface border border-border hover:border-accent/50 transition-all group flex flex-col justify-between shadow-sm relative overflow-hidden"
            >
              ...
              <div className="mt-5 pt-4 border-t border-border/40 font-mono text-xs text-text-primary bg-canvas/70 px-3.5 py-2.5 rounded-xl border border-border/50 flex items-center justify-between">
                <span className="text-text-secondary text-[11px]">Formula Acuan:</span>
      ...
      <div id="solvers" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {SECONDARY_FEATURES.map((item, idx) => {
          const ItemIcon = item.icon;
          return (
            <div
              key={idx}
              className="p-5 rounded-xl bg-surface/50 border border-border hover:border-accent/30 transition-all flex flex-col gap-2.5"
            >
